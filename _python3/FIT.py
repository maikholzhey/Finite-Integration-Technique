...
class FIT():
	...
	def boundaryIndicesBOR(self, boundaryConditions):
		"""
		Boundary Indices:
			same as 'boundaryIndices' bzt for rotational body (rz-code)
	...
	def fieldMonitor(self, data, dataMonitor, frequency, lenTimeSignal, stepIndex, dtmax):
		...
		dataMonitor is returned and needs to be stored. If it is an attribute of a FIT object this function would not automatically modify the content of the dataMonitor containter
...
def main():
	...
	main frame protected execution (means: only runs if directly executed, not at import modul statements)
	"""
	# clock the execution
	startTime = time.time()
	# create instance
